...
#[derive(Clone, Debug)]
pub struct PhysicalConstantExpr(pub Expr);

impl PhysicalConstantExpr {
    pub fn new(value: Value) -> Self {
        let typ = ConstantType::get_data_type_from_value(&value);
        Self::new_with_type(value, typ)
    }
    ...
    pub fn new_with_type(value: Value, typ: ConstantType) -> Self {
        PhysicalConstantExpr(Expr(
            RelNode {
                typ: OptRelNodeTyp::PhysicalConstant(typ),
                children: vec![],
                data: Some(value),
            }
            .into(),
    ...
    pub fn constant_type(&self) -> ConstantType {
        if let OptRelNodeTyp::PhysicalConstant(typ) = self.0.typ() {
            typ
        } else {
            panic!("not a constant")
        }
...
impl OptRelNode for PhysicalConstantExpr {
    fn into_rel_node(self) -> OptRelNodeRef {
        self.0.into_rel_node()
    }

    fn from_rel_node(rel_node: OptRelNodeRef) -> Option<Self> {
        if let OptRelNodeTyp::PhysicalConstant(_) = rel_node.typ {
            return Expr::from_rel_node(rel_node).map(Self);
        }
        None
    }
