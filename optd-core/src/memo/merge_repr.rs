...
/// A specialized Union-Find data structure for tracking group or goal
/// representatives caused by merges.
///
/// Implements union-find with path compression for O(α(n)) time complexity.
...
impl<T: Eq + Hash + Clone> Representative<T> {
    ...
    ///
    /// This is an immutable find operation that does not compress paths.
    ...
    /// Merges the sets containing `old` and `new`, forcing `new` to be
    /// the representative.
