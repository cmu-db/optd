...
#[async_recursion]
pub async fn egest_best_plan<M>(
    memo: &M,
    ...
    M: Memoize,
{
    ...
    let child_results = try_join_all(
        expr.0
            .children
            .iter()
            .map(|child| egest_child_plan(memo, child)),
    )
    .await?;
    ...
    M: Memoize,
{
    match child {
        Singleton(goal) => {
            ...
            let best_expr = match memo.get_best_optimized_physical_expr(goal).await? {
                Some(expr) => expr,
                None => return Ok(None),
            };

            // Recursively egest the plan for this expression
            let plan = match egest_best_plan(memo, &best_expr).await? {
                Some(plan) => plan,
                None => return Ok(None),
            };

            Ok(Some(Singleton(Arc::new(plan))))
        }
        VarLength(goals) => {
            ...
            let plans = match all_plans.into_iter().collect::<Option<Vec<_>>>() {
                Some(plans) => plans,
                None => return Ok(None),
            };
