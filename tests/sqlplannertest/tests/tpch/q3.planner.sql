...
Limit { .skip: 0::bigint, .fetch: 10::bigint, (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1), (.cardinality): 0.00 }
└── OrderBy { ordering_exprs: [ revenue(#5.1) DESC, o_orderdate(#5.2) ASC ], (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1), (.cardinality): 0.00 }
    └── Project { .table_index: 5, .projections: [ l_orderkey(#3.0), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#4.0), o_orderdate(#2.4), o_shippriority(#2.7) ], (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1), (.cardinality): 0.00 }
...
Limit { .skip: 0::bigint, .fetch: 10::bigint, (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1), (.cardinality): 0.00 }
└── EnforcerSort { tuple_ordering: [(#5.1, Desc), (#5.2, Asc)], (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1), (.cardinality): 0.00 }
    └── Project
        ├── .table_index: 5
        ├── .projections: [ l_orderkey(#3.0), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#4.0), o_orderdate(#2.4), o_shippriority(#2.7) ]
        ├── (.output_columns): l_orderkey(#5.0), o_orderdate(#5.2), o_shippriority(#5.3), revenue(#5.1)
        ├── (.cardinality): 0.00
        └── Aggregate
            ...
            ├── (.output_columns): l_orderkey(#3.0), o_orderdate(#2.4), o_shippriority(#2.7), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#4.0)
            ├── (.cardinality): 0.00
            └── Join
                ├── .join_type: Inner
                ├── .implementation: None
