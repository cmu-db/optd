...
OrderBy { ordering_exprs: [ `__internal_#5`.`l_shipmode`(#5.0) ASC ], (.output_columns): `__internal_#5`.`high_priority_orders`(#5.1), `__internal_#5`.`l_shipmode`(#5.0), `__internal_#5`.`low_priority_orders`(#5.2), (.cardinality): 0.00 }
└── Project { .table_index: 5, .projections: [ `lineitem`.`l_shipmode`(#2.14), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority = Utf8("1-URGENT") OR orders.o_orderpriority = Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.0), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority != Utf8("1-URGENT") AND orders.o_orderpriority != Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.1) ], (.output_columns): `__internal_#5`.`high_priority_orders`(#5.1), `__internal_#5`.`l_shipmode`(#5.0), `__internal_#5`.`low_priority_orders`(#5.2), (.cardinality): 0.00 }
    └── Aggregate
        ├── .key_table_index: 3
        ...
        ├── (.output_columns): `__internal_#3`.`l_shipmode`(#3.0), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority != Utf8("1-URGENT") AND orders.o_orderpriority != Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.1), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority = Utf8("1-URGENT") OR orders.o_orderpriority = Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.0)
        ├── (.cardinality): 0.00
        └── Join
            ├── .join_type: Inner
            ├── .implementation: None
            ├── .join_cond: (`orders`.`o_orderkey`(#1.0) = `lineitem`.`l_orderkey`(#2.0))
            ├── (.output_columns): `lineitem`.`l_comment`(#2.15), `lineitem`.`l_commitdate`(#2.11), `lineitem`.`l_discount`(#2.6), `lineitem`.`l_extendedprice`(#2.5), `lineitem`.`l_linenumber`(#2.3), `lineitem`.`l_linestatus`(#2.9), `lineitem`.`l_orderkey`(#2.0), `lineitem`.`l_partkey`(#2.1), `lineitem`.`l_quantity`(#2.4), `lineitem`.`l_receiptdate`(#2.12), `lineitem`.`l_returnflag`(#2.8), `lineitem`.`l_shipdate`(#2.10), `lineitem`.`l_shipinstruct`(#2.13), `lineitem`.`l_shipmode`(#2.14), `lineitem`.`l_suppkey`(#2.2), `lineitem`.`l_tax`(#2.7), `orders`.`o_clerk`(#1.6), `orders`.`o_comment`(#1.8), `orders`.`o_custkey`(#1.1), `orders`.`o_orderdate`(#1.4), `orders`.`o_orderkey`(#1.0), `orders`.`o_orderpriority`(#1.5), `orders`.`o_orderstatus`(#1.2), `orders`.`o_shippriority`(#1.7), `orders`.`o_totalprice`(#1.3)
            ├── (.cardinality): 0.00
...
EnforcerSort { tuple_ordering: [(#5.0, Asc)], (.output_columns): `__internal_#5`.`high_priority_orders`(#5.1), `__internal_#5`.`l_shipmode`(#5.0), `__internal_#5`.`low_priority_orders`(#5.2), (.cardinality): 0.00 }
└── Project
    ├── .table_index: 5
    ├── .projections: [ `lineitem`.`l_shipmode`(#2.14), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority = Utf8("1-URGENT") OR orders.o_orderpriority = Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.0), `__internal_#4`.`sum(CASE WHEN orders.o_orderpriority != Utf8("1-URGENT") AND orders.o_orderpriority != Utf8("2-HIGH") THEN Int64(1) ELSE Int64(0) END)`(#4.1) ]
    ├── (.output_columns): `__internal_#5`.`high_priority_orders`(#5.1), `__internal_#5`.`l_shipmode`(#5.0), `__internal_#5`.`low_priority_orders`(#5.2)
    ├── (.cardinality): 0.00
    └── Aggregate
        ├── .key_table_index: 3
        ├── .aggregate_table_index: 4
