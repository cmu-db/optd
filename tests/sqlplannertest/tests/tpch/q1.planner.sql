...
Limit { .skip: 0::bigint, .fetch: 3::bigint, (.output_columns): avg_disc(#4.8), avg_price(#4.7), avg_qty(#4.6), count_order(#4.9), l_linestatus(#4.1), l_returnflag(#4.0), sum_base_price(#4.3), sum_charge(#4.5), sum_disc_price(#4.4), sum_qty(#4.2), (.cardinality): 0.00 }
└── OrderBy { ordering_exprs: [ l_returnflag(#4.0) ASC, l_linestatus(#4.1) ASC ], (.output_columns): avg_disc(#4.8), avg_price(#4.7), avg_qty(#4.6), count_order(#4.9), l_linestatus(#4.1), l_returnflag(#4.0), sum_base_price(#4.3), sum_charge(#4.5), sum_disc_price(#4.4), sum_qty(#4.2), (.cardinality): 0.00 }
    └── Project
        ├── .table_index: 4
...
Limit { .skip: 0::bigint, .fetch: 3::bigint, (.output_columns): avg_disc(#4.8), avg_price(#4.7), avg_qty(#4.6), count_order(#4.9), l_linestatus(#4.1), l_returnflag(#4.0), sum_base_price(#4.3), sum_charge(#4.5), sum_disc_price(#4.4), sum_qty(#4.2), (.cardinality): 0.00 }
└── EnforcerSort { tuple_ordering: [(#4.0, Asc), (#4.1, Asc)], (.output_columns): avg_disc(#4.8), avg_price(#4.7), avg_qty(#4.6), count_order(#4.9), l_linestatus(#4.1), l_returnflag(#4.0), sum_base_price(#4.3), sum_charge(#4.5), sum_disc_price(#4.4), sum_qty(#4.2), (.cardinality): 0.00 }
    └── Project
        ├── .table_index: 4
        ├── .projections: [ l_returnflag(#2.9), l_linestatus(#2.10), sum(lineitem.l_quantity)(#3.0), sum(lineitem.l_extendedprice)(#3.1), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#3.2), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)(#3.3), avg(lineitem.l_quantity)(#3.4), avg(lineitem.l_extendedprice)(#3.5), avg(lineitem.l_discount)(#3.6), count(Int64(1))(#3.7) ]
        ├── (.output_columns): avg_disc(#4.8), avg_price(#4.7), avg_qty(#4.6), count_order(#4.9), l_linestatus(#4.1), l_returnflag(#4.0), sum_base_price(#4.3), sum_charge(#4.5), sum_disc_price(#4.4), sum_qty(#4.2)
