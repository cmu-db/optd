...
└── Project { .table_index: 8, .projections: [ nation.n_name(#5.1), __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0) ], (.output_columns): __internal_#8.nation(#8.0), __internal_#8.revenue(#8.1), (.cardinality): 0.20 }
    └── Aggregate { .aggregate_table_index: 7, .implementation: None, .exprs: sum(lineitem.l_extendedprice(#3.5) * CAST (1::bigint AS Decimal128(20, 0)) - lineitem.l_discount(#3.6)), .keys: [ nation.n_name(#5.1) ], (.output_columns): __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0), nation.n_name(#5.1), (.cardinality): 0.20 }
        └── Select
