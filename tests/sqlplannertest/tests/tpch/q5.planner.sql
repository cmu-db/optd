...
└── Project { .table_index: 8, .projections: [ nation.n_name(#5.1), __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0) ], (.output_columns): __internal_#8.nation(#8.0), __internal_#8.revenue(#8.1), (.cardinality): 0.00 }
    └── Aggregate { .aggregate_table_index: 7, .implementation: None, .exprs: sum(lineitem.l_extendedprice(#3.5) * CAST (1::bigint AS Decimal128(20, 0)) - lineitem.l_discount(#3.6)), .keys: [ nation.n_name(#5.1) ], (.output_columns): __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0), nation.n_name(#5.1), (.cardinality): 0.00 }
        └── Select
...
└── Project { .table_index: 8, .projections: [ nation.n_name(#5.1), __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0) ], (.output_columns): __internal_#8.nation(#8.0), __internal_#8.revenue(#8.1), (.cardinality): 0.00 }
    └── Aggregate { .aggregate_table_index: 7, .implementation: None, .exprs: sum(lineitem.l_extendedprice(#3.5) * CAST (1::bigint AS Decimal128(20, 0)) - lineitem.l_discount(#3.6)), .keys: [ nation.n_name(#5.1) ], (.output_columns): __internal_#7.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)(#7.0), nation.n_name(#5.1), (.cardinality): 0.00 }
        └── Join
            ...
            │   │   ├── (.cardinality): 0.00
            │   │   ├── Join { .join_type: Inner, .implementation: None, .join_cond: lineitem.l_orderkey(#3.0) = orders.o_orderkey(#2.0), (.output_columns): customer.c_custkey(#1.0), customer.c_nationkey(#1.3), lineitem.l_discount(#3.6), lineitem.l_extendedprice(#3.5), lineitem.l_orderkey(#3.0), lineitem.l_suppkey(#3.2), orders.o_custkey(#2.1), orders.o_orderdate(#2.4), orders.o_orderkey(#2.0), (.cardinality): 0.00 }
            ...
            │   │   │   │   └── Select { .predicate: (orders.o_orderdate(#2.4) >= CAST (2023-01-01::utf8 AS Date32)) AND (orders.o_orderdate(#2.4) < CAST (2024-01-01::utf8 AS Date32)), (.output_columns): orders.o_custkey(#2.1), orders.o_orderdate(#2.4), orders.o_orderkey(#2.0), (.cardinality): 0.00 }
            ...
            └── Select { .predicate: region.r_name(#6.1) = CAST (Asia::utf8 AS Utf8View), (.output_columns): region.r_name(#6.1), region.r_regionkey(#6.0), (.cardinality): 0.00 }
