...
OrderBy { ordering_exprs: [ `__internal_#9`.`revenue`(#9.1) DESC ], (.output_columns): `__internal_#9`.`nation`(#9.0), `__internal_#9`.`revenue`(#9.1), (.cardinality): 1.00 }
└── Project { .table_index: 9, .projections: [ `nation`.`n_name`(#5.1), `__internal_#8`.`sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)`(#8.0) ], (.output_columns): `__internal_#9`.`nation`(#9.0), `__internal_#9`.`revenue`(#9.1), (.cardinality): 1.00 }
    └── Aggregate { .key_table_index: 7, .aggregate_table_index: 8, .implementation: None, .exprs: sum(`lineitem`.`l_extendedprice`(#3.5) * 1::decimal128(20, 0) - `lineitem`.`l_discount`(#3.6)), .keys: [ `nation`.`n_name`(#5.1) ], (.output_columns): `__internal_#7`.`n_name`(#7.0), `__internal_#8`.`sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)`(#8.0), (.cardinality): 1.00 }
        └── Join
            ├── .join_type: Inner
            ├── .implementation: None
            ...
            │   │   │   │   └── Select { .predicate: (`orders`.`o_orderdate`(#2.4) >= 1994-01-01::date32) AND (`orders`.`o_orderdate`(#2.4) < 1995-01-01::date32), (.output_columns): `orders`.`o_clerk`(#2.6), `orders`.`o_comment`(#2.8), `orders`.`o_custkey`(#2.1), `orders`.`o_orderdate`(#2.4), `orders`.`o_orderkey`(#2.0), `orders`.`o_orderpriority`(#2.5), `orders`.`o_orderstatus`(#2.2), `orders`.`o_shippriority`(#2.7), `orders`.`o_totalprice`(#2.3), (.cardinality): 0.00 }
            ...
            └── Select { .predicate: `region`.`r_name`(#6.1) = ASIA::utf8_view, (.output_columns): `region`.`r_comment`(#6.2), `region`.`r_name`(#6.1), `region`.`r_regionkey`(#6.0), (.cardinality): 0.00 }
...
EnforcerSort { tuple_ordering: [(#9.1, Desc)], (.output_columns): `__internal_#9`.`nation`(#9.0), `__internal_#9`.`revenue`(#9.1), (.cardinality): 1.00 }
└── Project { .table_index: 9, .projections: [ `nation`.`n_name`(#5.1), `__internal_#8`.`sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)`(#8.0) ], (.output_columns): `__internal_#9`.`nation`(#9.0), `__internal_#9`.`revenue`(#9.1), (.cardinality): 1.00 }
    └── Aggregate { .key_table_index: 7, .aggregate_table_index: 8, .implementation: None, .exprs: sum(`lineitem`.`l_extendedprice`(#3.5) * 1::decimal128(20, 0) - `lineitem`.`l_discount`(#3.6)), .keys: [ `nation`.`n_name`(#5.1) ], (.output_columns): `__internal_#7`.`n_name`(#7.0), `__internal_#8`.`sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)`(#8.0), (.cardinality): 1.00 }
        └── Join
            ├── .join_type: Inner
            ...
            │   │   │   │   └── Select { .predicate: (`orders`.`o_orderdate`(#2.4) >= 1994-01-01::date32) AND (`orders`.`o_orderdate`(#2.4) < 1995-01-01::date32), (.output_columns): `orders`.`o_custkey`(#2.1), `orders`.`o_orderdate`(#2.4), `orders`.`o_orderkey`(#2.0), (.cardinality): 0.00 }
