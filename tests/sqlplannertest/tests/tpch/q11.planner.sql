...
OrderBy { ordering_exprs: "__#12.value"(#12.1) DESC, (.output_columns): [ "__#12.ps_partkey"(#12.0), "__#12.value"(#12.1) ], (.cardinality): 0.00 }
└── Project
    ├── .table_index: 12
    ├── .projections: [ "partsupp.ps_partkey"(#1.0), "__#5.sum(partsupp.ps_supplycost * partsupp.ps_availqty)"(#5.0) ]
    ├── (.output_columns): [ "__#12.ps_partkey"(#12.0), "__#12.value"(#12.1) ]
    ├── (.cardinality): 0.00
    └── DependentJoin
        ├── .join_type: Inner
        ├── .join_cond: CAST ("__#5.sum(partsupp.ps_supplycost * partsupp.ps_availqty)"(#5.0) AS Decimal128(38, 15)) > "__#11.sum(partsupp.ps_supplycost * partsupp.ps_availqty) * Float64(0.0001)"(#11.0)
        ├── (.output_columns): [ "__#11.sum(partsupp.ps_supplycost * partsupp.ps_availqty) * Float64(0.0001)"(#11.0), "__#4.ps_partkey"(#4.0), "__#5.sum(partsupp.ps_supplycost * partsupp.ps_availqty)"(#5.0) ]
        ├── (.cardinality): 0.00
        ├── Aggregate
        │   ├── .key_table_index: 4
        ...
        │   ├── (.cardinality): 0.00
        │   └── Select
        │       ├── .predicate: ("partsupp.ps_suppkey"(#1.1) = "supplier.s_suppkey"(#2.0)) AND ("supplier.s_nationkey"(#2.3) = "nation.n_nationkey"(#3.0)) AND ("nation.n_name"(#3.1) = CAST ('CHINA'::utf8 AS Utf8View))
        │       ├── (.output_columns):
        ...
        └── Project
            ...
            └── Aggregate
                ...
                ├── (.cardinality): 1.00
                └── Select
                    ├── .predicate: ("partsupp.ps_suppkey"(#6.1) = "supplier.s_suppkey"(#7.0)) AND ("supplier.s_nationkey"(#7.3) = "nation.n_nationkey"(#8.0)) AND ("nation.n_name"(#8.1) = CAST ('CHINA'::utf8 AS Utf8View))
                    ├── (.output_columns):
                    ...
                    └── Join
                        ...
                        │   │   ├── .table_index: 6
                        │   │   ├── .implementation: None
                        │   │   ├── (.output_columns): [ "partsupp.ps_availqty"(#6.2), "partsupp.ps_comment"(#6.4), "partsupp.ps_partkey"(#6.0), "partsupp.ps_suppkey"(#6.1), "partsupp.ps_supplycost"(#6.3) ]
                        │   │   └── (.cardinality): 0.00
                        │   └── Get
                        │       ├── .data_source_id: 4
                        │       ├── .table_index: 7
...
EnforcerSort { tuple_ordering: [(#12.1, Desc)], (.output_columns): [ "__#12.ps_partkey"(#12.0), "__#12.value"(#12.1) ], (.cardinality): 0.00 }
└── Project
    ├── .table_index: 12
    ├── .projections: [ "partsupp.ps_partkey"(#1.0), "__#5.sum(partsupp.ps_supplycost * partsupp.ps_availqty)"(#5.0) ]
    ├── (.output_columns): [ "__#12.ps_partkey"(#12.0), "__#12.value"(#12.1) ]
    ...
    └── Join
        ...
        ├── .join_cond: CAST ("__#5.sum(partsupp.ps_supplycost * partsupp.ps_availqty)"(#5.0) AS Decimal128(38, 15)) > "__#11.sum(partsupp.ps_supplycost * partsupp.ps_availqty) * Float64(0.0001)"(#11.0)
        ├── (.output_columns):
        │   ┌── "__#11.sum(partsupp.ps_supplycost * partsupp.ps_availqty) * Float64(0.0001)"(#11.0)
        │   ├── "__#4.ps_partkey"(#4.0)
