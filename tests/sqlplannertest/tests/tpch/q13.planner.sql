...
└── Project
    ├── .table_index: 8
    ├── .projections: [ c_orders.c_count(#6.1), __internal_#7.count(Int64(1))(#7.0) ]
    ├── (.output_columns): __internal_#8.c_count(#8.0), __internal_#8.custdist(#8.1)
    ├── (.cardinality): 0.20
    └── Aggregate
        ├── .aggregate_table_index: 7
        ├── .implementation: None
        ├── .exprs: [ count(1::bigint) ]
        ├── .keys: [ c_orders.c_count(#6.1) ]
        ├── (.output_columns): __internal_#7.count(Int64(1))(#7.0), c_orders.c_count(#6.1)
        ├── (.cardinality): 0.20
        └── Remap { .table_index: 6, (.output_columns): c_orders.c_count(#6.1), c_orders.c_custkey(#6.0), (.cardinality): 0.20 }
            └── Project
                ├── .table_index: 5
                ├── .projections: [ customer.c_custkey(#1.0), __internal_#3.count(orders.o_orderkey)(#3.0) ]
                ├── (.output_columns): __internal_#5.c_count(#5.1), __internal_#5.c_custkey(#5.0)
                ├── (.cardinality): 0.20
                └── Aggregate
                    ...
                    ├── (.output_columns): __internal_#3.count(orders.o_orderkey)(#3.0), customer.c_custkey(#1.0)
                    ├── (.cardinality): 0.20
                    └── Join
                        ├── .join_type: Left
                        ├── .implementation: None
