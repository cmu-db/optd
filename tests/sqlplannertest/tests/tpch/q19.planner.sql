...
└── Aggregate { .key_table_index: 3, .aggregate_table_index: 4, .implementation: None, .exprs: sum("lineitem.l_extendedprice"(#1.5) * CAST (1::bigint AS Decimal128(20, 0)) - "lineitem.l_discount"(#1.6)), .keys: [], (.output_columns): "__#4.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)"(#4.0), (.cardinality): 1.00 }
    └── Select
        ├── .predicate: (("part.p_partkey"(#2.0) = "lineitem.l_partkey"(#1.1)) AND ("part.p_brand"(#2.3) = CAST ('Brand#12'::utf8 AS Utf8View)) AND ("part.p_container"(#2.6) IN [CAST ('SM CASE'::utf8 AS Utf8View), CAST ('SM BOX'::utf8 AS Utf8View), CAST ('SM PACK'::utf8 AS Utf8View), CAST ('SM PKG'::utf8 AS Utf8View)]) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) >= CAST (1::bigint AS Decimal128(22, 2))) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) <= CAST (11::bigint AS Decimal128(22, 2))) AND (CAST ("part.p_size"(#2.5) AS Int64) >= 1::bigint) AND (CAST ("part.p_size"(#2.5) AS Int64) <= 5::bigint) AND ("lineitem.l_shipmode"(#1.14) IN [CAST ('AIR'::utf8 AS Utf8View), CAST ('AIR REG'::utf8 AS Utf8View)]) AND ("lineitem.l_shipinstruct"(#1.13) = CAST ('DELIVER IN PERSON'::utf8 AS Utf8View))) OR (("part.p_partkey"(#2.0) = "lineitem.l_partkey"(#1.1)) AND ("part.p_brand"(#2.3) = CAST ('Brand#23'::utf8 AS Utf8View)) AND ("part.p_container"(#2.6) IN [CAST ('MED BAG'::utf8 AS Utf8View), CAST ('MED BOX'::utf8 AS Utf8View), CAST ('MED PKG'::utf8 AS Utf8View), CAST ('MED PACK'::utf8 AS Utf8View)]) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) >= CAST (10::bigint AS Decimal128(22, 2))) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) <= CAST (20::bigint AS Decimal128(22, 2))) AND (CAST ("part.p_size"(#2.5) AS Int64) >= 1::bigint) AND (CAST ("part.p_size"(#2.5) AS Int64) <= 10::bigint) AND ("lineitem.l_shipmode"(#1.14) IN [CAST ('AIR'::utf8 AS Utf8View), CAST ('AIR REG'::utf8 AS Utf8View)]) AND ("lineitem.l_shipinstruct"(#1.13) = CAST ('DELIVER IN PERSON'::utf8 AS Utf8View))) OR (("part.p_partkey"(#2.0) = "lineitem.l_partkey"(#1.1)) AND ("part.p_brand"(#2.3) = CAST ('Brand#34'::utf8 AS Utf8View)) AND ("part.p_container"(#2.6) IN [CAST ('LG CASE'::utf8 AS Utf8View), CAST ('LG BOX'::utf8 AS Utf8View), CAST ('LG PACK'::utf8 AS Utf8View), CAST ('LG PKG'::utf8 AS Utf8View)]) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) >= CAST (20::bigint AS Decimal128(22, 2))) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) <= CAST (30::bigint AS Decimal128(22, 2))) AND (CAST ("part.p_size"(#2.5) AS Int64) >= 1::bigint) AND (CAST ("part.p_size"(#2.5) AS Int64) <= 15::bigint) AND ("lineitem.l_shipmode"(#1.14) IN [CAST ('AIR'::utf8 AS Utf8View), CAST ('AIR REG'::utf8 AS Utf8View)]) AND ("lineitem.l_shipinstruct"(#1.13) = CAST ('DELIVER IN PERSON'::utf8 AS Utf8View)))
        ...
        ├── (.cardinality): 0.00
        └── Join { .join_type: Inner, .implementation: None, .join_cond: , (.output_columns): [ "lineitem.l_comment"(#1.15), "lineitem.l_commitdate"(#1.11), "lineitem.l_discount"(#1.6), "lineitem.l_extendedprice"(#1.5), "lineitem.l_linenumber"(#1.3), "lineitem.l_linestatus"(#1.9), "lineitem.l_orderkey"(#1.0), "lineitem.l_partkey"(#1.1), "lineitem.l_quantity"(#1.4), "lineitem.l_receiptdate"(#1.12), "lineitem.l_returnflag"(#1.8), "lineitem.l_shipdate"(#1.10), "lineitem.l_shipinstruct"(#1.13), "lineitem.l_shipmode"(#1.14), "lineitem.l_suppkey"(#1.2), "lineitem.l_tax"(#1.7), "part.p_brand"(#2.3), "part.p_comment"(#2.8), "part.p_container"(#2.6), "part.p_mfgr"(#2.2), "part.p_name"(#2.1), "part.p_partkey"(#2.0), "part.p_retailprice"(#2.7), "part.p_size"(#2.5), "part.p_type"(#2.4) ], (.cardinality): 0.00 }
...
└── Aggregate { .key_table_index: 3, .aggregate_table_index: 4, .implementation: None, .exprs: sum("lineitem.l_extendedprice"(#1.5) * 1::decimal128(20, 0) - "lineitem.l_discount"(#1.6)), .keys: [], (.output_columns): "__#4.sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)"(#4.0), (.cardinality): 1.00 }
    └── Join
        ...
        ├── (.cardinality): 0.00
        ├── Select { .predicate: ("lineitem.l_shipmode"(#1.14) IN ['AIR'::utf8_view, 'AIR REG'::utf8_view]) AND ("lineitem.l_shipinstruct"(#1.13) = 'DELIVER IN PERSON'::utf8_view), (.output_columns): [ "lineitem.l_discount"(#1.6), "lineitem.l_extendedprice"(#1.5), "lineitem.l_partkey"(#1.1), "lineitem.l_quantity"(#1.4), "lineitem.l_shipinstruct"(#1.13), "lineitem.l_shipmode"(#1.14) ], (.cardinality): 0.00 }
