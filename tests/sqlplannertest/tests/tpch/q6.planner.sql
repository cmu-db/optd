...
└── Aggregate { .key_table_index: 2, .aggregate_table_index: 3, .implementation: None, .exprs: sum("lineitem.l_extendedprice"(#1.5) * "lineitem.l_discount"(#1.6)), .keys: [], (.output_columns): "__#3.sum(lineitem.l_extendedprice * lineitem.l_discount)"(#3.0), (.cardinality): 1.00 }
    └── Select
        ├── .predicate: ("lineitem.l_shipdate"(#1.10) >= CAST ('2023-01-01'::utf8 AS Date32)) AND ("lineitem.l_shipdate"(#1.10) < CAST ('2024-01-01'::utf8 AS Date32)) AND ((CAST ("lineitem.l_discount"(#1.6) AS Decimal128(30, 15)) >= CAST (0.05::float64 AS Decimal128(30, 15))) AND (CAST ("lineitem.l_discount"(#1.6) AS Decimal128(30, 15)) <= CAST (0.07::float64 AS Decimal128(30, 15)))) AND (CAST ("lineitem.l_quantity"(#1.4) AS Decimal128(22, 2)) < CAST (24::bigint AS Decimal128(22, 2)))
        ├── (.output_columns):
