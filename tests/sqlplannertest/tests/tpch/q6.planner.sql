...
Project { .table_index: 4, .projections: sum(lineitem.l_extendedprice * lineitem.l_discount)(#3.0), (.output_columns): revenue_loss(#4.0), (.cardinality): 1.00 }
└── Aggregate { .key_table_index: 2, .aggregate_table_index: 3, .implementation: None, .exprs: [ sum(l_extendedprice(#1.5) * l_discount(#1.6)) ], .keys: [], (.output_columns): sum(lineitem.l_extendedprice * lineitem.l_discount)(#3.0), (.cardinality): 1.00 }
    └── Select
...
└── Aggregate
    ...
    ├── .implementation: None
    ├── .exprs: [ sum(l_extendedprice(#1.5) * l_discount(#1.6)) ]
    ├── .keys: []
    ├── (.output_columns): sum(lineitem.l_extendedprice * lineitem.l_discount)(#3.0)
    ├── (.cardinality): 1.00
    └── Select
        ├── .predicate: (l_shipdate(#1.10) >= 2023-01-01::date32) AND (l_shipdate(#1.10) < 2024-01-01::date32) AND (l_discount(#1.6) >= 5::decimal128(15, 2)) AND (l_discount(#1.6) <= 7::decimal128(15, 2)) AND (l_quantity(#1.4) < 2400::decimal128(15, 2))
