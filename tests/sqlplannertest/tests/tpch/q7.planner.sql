        ...
        └── Remap { .table_index: 10, (.output_columns): `shipping`.`cust_nation`(#10.1), `shipping`.`l_year`(#10.2), `shipping`.`supp_nation`(#10.0), `shipping`.`volume`(#10.3), (.cardinality): 0.00 }
            └── Project { .table_index: 9, .projections: [ `n1`.`n_name`(#6.1), `n2`.`n_name`(#8.1), date_part(YEAR::utf8, `lineitem`.`l_shipdate`(#2.10)), `lineitem`.`l_extendedprice`(#2.5) * 1::decimal128(20, 0) - `lineitem`.`l_discount`(#2.6) ], (.output_columns): `__internal_#9`.`cust_nation`(#9.1), `__internal_#9`.`l_year`(#9.2), `__internal_#9`.`supp_nation`(#9.0), `__internal_#9`.`volume`(#9.3), (.cardinality): 0.00 }
                └── Join
                    ...
                    │   │   │   │   └── Select { .predicate: (`lineitem`.`l_shipdate`(#2.10) >= 1995-01-01::date32) AND (`lineitem`.`l_shipdate`(#2.10) <= 1996-12-31::date32), (.output_columns): `lineitem`.`l_comment`(#2.15), `lineitem`.`l_commitdate`(#2.11), `lineitem`.`l_discount`(#2.6), `lineitem`.`l_extendedprice`(#2.5), `lineitem`.`l_linenumber`(#2.3), `lineitem`.`l_linestatus`(#2.9), `lineitem`.`l_orderkey`(#2.0), `lineitem`.`l_partkey`(#2.1), `lineitem`.`l_quantity`(#2.4), `lineitem`.`l_receiptdate`(#2.12), `lineitem`.`l_returnflag`(#2.8), `lineitem`.`l_shipdate`(#2.10), `lineitem`.`l_shipinstruct`(#2.13), `lineitem`.`l_shipmode`(#2.14), `lineitem`.`l_suppkey`(#2.2), `lineitem`.`l_tax`(#2.7), (.cardinality): 0.00 }
                    ...
                    │       └── Select { .predicate: (`nation`.`n_name`(#5.1) = FRANCE::utf8_view) OR (`nation`.`n_name`(#5.1) = GERMANY::utf8_view), (.output_columns): `nation`.`n_comment`(#5.3), `nation`.`n_name`(#5.1), `nation`.`n_nationkey`(#5.0), `nation`.`n_regionkey`(#5.2), (.cardinality): 0.00 }
                    ...
                    └── Remap { .table_index: 8, (.output_columns): `n2`.`n_comment`(#8.3), `n2`.`n_name`(#8.1), `n2`.`n_nationkey`(#8.0), `n2`.`n_regionkey`(#8.2), (.cardinality): 0.00 }
                        └── Select { .predicate: (`nation`.`n_name`(#7.1) = GERMANY::utf8_view) OR (`nation`.`n_name`(#7.1) = FRANCE::utf8_view), (.output_columns): `nation`.`n_comment`(#7.3), `nation`.`n_name`(#7.1), `nation`.`n_nationkey`(#7.0), `nation`.`n_regionkey`(#7.2), (.cardinality): 0.00 }
