...
fn convert_plan(
    ...
    match plan {
        ...
        LogicalPlan::SubqueryAlias(alias) => {
            ...
            Ok(OperatorData::Rename(optd_core::Rename {
                alias: alias_name,
                defs,
                input,
            })
...
fn convert_join(
    ...
    let mut exprs: Vec<optd_core::Expr> = join
        .on
        .iter()
        .map(|(l, r)| {
...
fn convert_expr(
    ...
    let data = match expr {
        ...
        DFExpr::InList(inlist) => {
            ...
            let exprs: Vec<optd_core::Expr> = inlist
                .list
                .iter()
                .map(|item| {
        ...
        DFExpr::ScalarFunction(sf) => {
            let args: Vec<optd_core::Expr> = sf
                .args
                .iter()
                .map(|a| convert_expr(a, ctx, bindings))
                .collect::<FromDFResult<_>>()?;
            ExprData::ScalarFunction {
                function: optd_core::ScalarFunction::extension(sf.func.name()),
                args,
            }
        }
        DFExpr::Case(Case {
...
mod tests {
    ...
    use optd_core::{OperatorData, QueryContext};
    use std::sync::Arc;
