use crate::{
    ...
    engine::utils::{
        cache::StreamCache,
        errors::Error,
        streams::{ValueStream, VecValueStream},
    },
...
pub(super) fn propagate_error(e: Error) -> ValueStream {
            ...
            if !has_more {
                ...
                first
                    ...
                    .flat_map_unordered(None, {
                        capture!([cache], move |first_result| match first_result {
                            Ok(first_val) => {
                                capture!([cache, remaining_items, context], async move {
    ...
    pub(crate) fn evaluate(&self, context: Context) -> ValueStream {
        match self {
            ...
            Binary(left, op, right) => {
                ...
                .map(capture!([op], move |combo_result| {
                    combo_result.map(|mut values| {
                        ...
                        eval_binary_op(left_val, &op.clone(), right_val)
                    })
                }));
            ...
            Unary(op, expr) => {
                let stream = expr
                    .evaluate(context.clone())
                    .map(capture!([op], move |expr_result| expr_result
                        .map(|value| eval_unary_op(&op, value))))
                    .boxed();
            ...
            Call(fun, args) => {
                ...
                let stream = fun_stream.flat_map_unordered(
                    None,
                    capture!([args], move |fun_result| {
                        let context = context.clone();
