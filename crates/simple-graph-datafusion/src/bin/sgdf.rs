...
use simple_graph::FreeColumns;
use simple_graph::QueryFormatConfig;
use simple_graph_datafusion::runner::{RunnerOutput, SimpleGraphRunner};
use simple_graph_datafusion::setup::{session_context_with_information_schema, setup_tpch_session};

#[derive(Debug, Parser)]
#[command(
    name = "sgdf",
    about = "Run SQL through the simple-graph DataFusion bridge"
)]
struct Args {
    ...
    /// Print box-formatted simple-graph optimizer steps before executing each statement.
    #[arg(long)]
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let runner = SimpleGraphRunner::new(session);
...
async fn repl(runner: &SimpleGraphRunner, options: ExecuteOptions) -> Result<(), Box<dyn Error>> {
...
fn log_explain_steps(runner: &SimpleGraphRunner, statement: &str) -> Result<(), Box<dyn Error>> {
