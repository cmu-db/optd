//! Profiling helpers for simple-graph optimizer passes using DataFusion SQL input.
...
use simple_graph::{Operator, OperatorData, optimize::join_ordering::collect_join_group_roots};

use crate::from_df::{FromDFError, from_logical_plan};

/// Error produced while building or profiling a simple-graph optimizer input.
#[derive(Debug)]
pub enum ProfilingError {
    DataFusion(datafusion::error::DataFusionError),
    FromDataFusion(FromDFError),
    Optimize(simple_graph::OptimizeError),
}
...
impl From<simple_graph::OptimizeError> for ProfilingError {
    fn from(error: simple_graph::OptimizeError) -> Self {
        Self::Optimize(error)
    }
}
...
/// Profiles simple-graph PredicatePushdown + JoinOrdering for one DataFusion SQL query.
///
